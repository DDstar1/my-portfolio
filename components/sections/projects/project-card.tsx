...
export default function ProjectCard({ project }: props) {
  ...
  const { scrollYProgress } = useScroll({
    ...
    offset: ["start end", "end start"], // Better offset for full card visibility
  });

  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    console.log(`Project ${project.title} scroll progress:`, latest);
  });

  return (
    <div
      ref={cardContainer}
      className="w-full h-screen max-w-[650px] rounded-2xl bg-muted border sticky top-8"
    >
